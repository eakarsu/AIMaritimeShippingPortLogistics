...
// Route: /api/gap-shipper
// Feature: Shipper
// Description: Shipper portal (cargo tracking, ETAs, docs)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Shipper. Shipper portal (cargo tracking, ETAs, docs)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'shipper', title: 'Shipper', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'shipper', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['shipper']);
      return res.json({ slug: 'shipper', items: r.rows || [] });
    }
    return res.json({ slug: 'shipper', items: [] });
  } catch (e) {
    return res.json({ slug: 'shipper', items: [], error: (e && e.message) || 'Server error' });
  }
});
