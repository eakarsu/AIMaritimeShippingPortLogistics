...
// Route: /api/gap-environmental-compliance-checker
// Feature: Environmental Compliance Checker
// Description: (emissions, waste)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Environmental Compliance Checker. (emissions, waste)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Environmental Compliance Checker\nDescription: (emissions, waste)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['environmental-compliance-checker', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'environmental-compliance-checker', title: 'Environmental Compliance Checker', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'environmental-compliance-checker', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['environmental-compliance-checker']);
      return res.json({ slug: 'environmental-compliance-checker', items: r.rows || [] });
    }
    return res.json({ slug: 'environmental-compliance-checker', items: [] });
  } catch (e) {
    return res.json({ slug: 'environmental-compliance-checker', items: [], error: (e && e.message) || 'Server error' });
  }
});
