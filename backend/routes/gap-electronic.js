...
// Route: /api/gap-electronic
// Feature: Electronic
// Description: Electronic manifest submission (paperless customs)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Electronic. Electronic manifest submission (paperless customs)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'electronic', title: 'Electronic', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'electronic', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['electronic']);
      return res.json({ slug: 'electronic', items: r.rows || [] });
    }
    return res.json({ slug: 'electronic', items: [] });
  } catch (e) {
    return res.json({ slug: 'electronic', items: [], error: (e && e.message) || 'Server error' });
  }
});
