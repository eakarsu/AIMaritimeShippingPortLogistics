// Auto-generated feature route: demurrage-prediction
// Domain: demurrage analytics agent — Capability: predicting and proactively prioritizing demurrage cases
// Mounted under: /api/demurrage-prediction
...
// POST /api/demurrage-prediction/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a demurrage analytics agent. Your task is predicting and proactively prioritizing demurrage cases. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/demurrage-prediction/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/demurrage-prediction/:id — fetch one
...
// DELETE /api/demurrage-prediction/:id — discard a run
