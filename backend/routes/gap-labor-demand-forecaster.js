...
// Route: /api/gap-labor-demand-forecaster
// Feature: Labor Demand Forecaster
// Description: (stevedore/crane operator demand)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Labor Demand Forecaster. (stevedore/crane operator demand)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Labor Demand Forecaster\nDescription: (stevedore/crane operator demand)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['labor-demand-forecaster', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'labor-demand-forecaster', title: 'Labor Demand Forecaster', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'labor-demand-forecaster', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['labor-demand-forecaster']);
      return res.json({ slug: 'labor-demand-forecaster', items: r.rows || [] });
    }
    return res.json({ slug: 'labor-demand-forecaster', items: [] });
  } catch (e) {
    return res.json({ slug: 'labor-demand-forecaster', items: [], error: (e && e.message) || 'Server error' });
  }
});
