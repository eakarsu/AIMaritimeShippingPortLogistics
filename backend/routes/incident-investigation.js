// Auto-generated feature route: incident-investigation
// Domain: maritime incident investigator — Capability: combining vessel sensors, weather, and reports
// Mounted under: /api/incident-investigation
...
// POST /api/incident-investigation/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a maritime incident investigator. Your task is combining vessel sensors, weather, and reports. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/incident-investigation/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/incident-investigation/:id — fetch one
...
// DELETE /api/incident-investigation/:id — discard a run
