...
// Route: /api/gap-liability-risk-assessment
// Feature: Liability Risk Assessment
// Description: (cargo type + vessel age + weather)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Liability Risk Assessment. (cargo type + vessel age + weather)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Liability Risk Assessment\nDescription: (cargo type + vessel age + weather)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['liability-risk-assessment', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'liability-risk-assessment', title: 'Liability Risk Assessment', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'liability-risk-assessment', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['liability-risk-assessment']);
      return res.json({ slug: 'liability-risk-assessment', items: r.rows || [] });
    }
    return res.json({ slug: 'liability-risk-assessment', items: [] });
  } catch (e) {
    return res.json({ slug: 'liability-risk-assessment', items: [], error: (e && e.message) || 'Server error' });
  }
});
