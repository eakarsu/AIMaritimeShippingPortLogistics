// Auto-generated feature route: port-optimization-stream
// Domain: port operations optimizer — Capability: streaming berth, crane, and scheduling optimization
// Mounted under: /api/port-optimization-stream
...
// POST /api/port-optimization-stream/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a port operations optimizer. Your task is streaming berth, crane, and scheduling optimization. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/port-optimization-stream/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/port-optimization-stream/:id — fetch one
...
// DELETE /api/port-optimization-stream/:id — discard a run
